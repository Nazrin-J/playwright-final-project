...
```
Error: locator.click: Error: strict mode violation: locator('.modal-footer button.btn.btn-secondary') resolved to 4 elements:
    1) <button type="button" data-dismiss="modal" class="btn btn-secondary">Close</button> aka getByLabel('New message').getByText('Close')
    2) <button type="button" data-dismiss="modal" class="btn btn-secondary">Close</button> aka getByLabel('Sign up').getByText('Close')
    3) <button type="button" data-dismiss="modal" class="btn btn-secondary">Close</button> aka getByLabel('Log in').getByText('Close')
    4) <button type="button" data-dismiss="modal" class="btn btn-secondary">Close</button> aka locator('#videoModal').getByText('Close', { exact: true })

Call log:
  - waiting for locator('.modal-footer button.btn.btn-secondary')
      ...
      - generic [ref=e4]:
        ...
        - generic [ref=e9]:
          - generic [ref=e10]:
            ...
            - textbox "Username:" [ref=e12]: testdemo224
          - generic [ref=e13]:
            - generic [ref=e14]: "Password:"
            - textbox "Password:" [active] [ref=e15]: testdemopass235
        - generic [ref=e17]:
...
```ts
  ...
  7  | this.closebutton=page.locator('.modal-footer button.btn.btn-secondary')
  8  | 
  9  | }
  10 | 
  11 | async register()        
  ...
  14 | }
  15 | 
  16 | async signinguser()
  17 | {
  18 |     await this.signupuser.click()
  19 | }
  20 | 
  21 | async usersignupcredentials(user1,pass1)
  22 | {
  23 | 
  24 | 
  25 |     await this.username.fill(user1)
  26 |     await this.password.fill(pass1)
  27 | }
  28 | 
  29 | 
  30 | async closeregister()
  31 | {
